...
class steamAuthentification {
    private $settings = array(
        "apikey" => "0B71AE34455C197B52A50AFF8E9581AD", // Get yours today from http://steamcommunity.com/dev/apikey
        "domainname" => "strategyinrush.com", // Displayed domain in the login-screen
        "loginpage" => "", // Returns to last page if not set
        "logoutpage" => "",
        "skipAPI" => false // true = dont get the data from steam, just return the steamid
    );

    function __construct() {
        if (session_id() == "") session_start();  // Start a session if none exists
        if ($this->settings["apikey"] == "") die("<b>SteamAuth:</b> Please supply a valid API-Key!");
        if ($this->settings["loginpage"] == "") $this->settings["loginpage"] = /* [ */ (!empty($_SERVER['HTTPS']) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['SCRIPT_NAME'];
     ...
     */
    function debug() {
        if (empty($_SESSION["steamdata"])) {
            $steamdata = 'Empty Steam Data(not logged in)';
        } else {
            $steamdata = $_SESSION["steamdata"];
        }

        echo "<h1>SteamAuth debug report</h1><hr><b>Settings-array:</b><br>";
        echo "<pre>".print_r($this->settings,true)."</pre>";
        echo "<br><br><b>Data:</b><br>";
        echo "<pre>".print_r($steamdata, true)."</pre>";
    }
}
?>
